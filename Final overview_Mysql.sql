...
CREATE TABLE Departments(department_id INT PRIMARY KEY NOT NULL,
                        ...
                        CREATE TABLE Location(location_id INT AUTO_INCREMENT PRIMARY KEY NOT NULL,	
						location VARCHAR(50));
...
--  Distinct Values:
-- query to retrieve distinct salaries from the Employees table.
...
SELECT employee_name , salary 'employee_salary' ,hire_date
FROM employees
...
SELECT employee_name , MIN(age) FROM employees
...
SELECT l.location , MAX(salary) 'Maximum Salary'
FROM employees e
...
SELECT designation ,AVG(salary) 'Average salary' FROM employees
...
SELECT * FROM employees;
SELECT  department_name,COUNT(*)'Employees' 
FROM employees e
...
select e.employee_name,gender,l.location,avg(age)
from employees e
join  location l on l.location_id = e.location_id
...
SELECT  d.department_name, COUNT(*) 'employee'
FROM employees e
